...
def main(args):
    ...
    for i,hist in enumerate(hists):
        ...
        searchstring =r'_mRp(\d+)_'
        res=re.search(searchstring, inname)
        m=int(res.group(1))
        #m=125
        #m=150
        #m=400
        ...
        for ip,parname in enumerate(parnames):
            ...
            ROOT.gStyle.SetTextSize(15) # mass text
            ROOT.gStyle.SetLegendTextSize(15) # legend
    ...
    for m in range(int(args.minmass), int(args.maxmass), int(args.spacing)):
        print(m)
