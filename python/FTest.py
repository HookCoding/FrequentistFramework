...
def main(args):
    ...
    parser.add_option('--pvalbin', dest='pvalbin', type=int, default=6, help='bin of the pval value in the chi2 histogram')
    ...
    for p in paths:
        ...
        h_chi2 = f.Get(folder+options.chi2hist)
        # h_pf   = f.Get(options.postfithist)
        h_res  = f.Get(folder+options.residualshist)
        #h_res.Rebin(4)
        # h_pf.SetDirectory(0)
    ...
    for i in range(len(l_res)):
        ...
        if "nlofit" in paths[i].lower():
            ...
            if "four" in paths[i].lower():
                ...
                p=8
            print(l_pval[i])
            legtext = "#splitline{%d-par fit}{#chi^{2}/n = %.1f/%.0f = %.2f, p = %.4f}" % (p, l_chi2[i], l_ndof[i], l_chi2[i]/l_ndof[i], l_pval[i]) 
    ...
    for i in range(len(l_chi2)-1):
        ...
        if "nlofit" in paths[i].lower():
            ...
            leg2.AddEntry(0, "p(F_{^{%d #rightarrow %d par}}) = %.4f" % (l_par[i], l_par[i+1], pF), "")
