...
def main(args):
    ...
    #ROOT.Math.MinimizerOptions.SetDefaultMinimizer("Minuit")  # or "Minuit2"
    ...
    signal = int(re.search(r'mRp(\d+)', args.folder).group(1)) # get signal mass value
    print(signal)

    range_min = signal*0.4 #signal*0.95-3*math.sqrt(signal)
    range_max = signal*1.4 #signal*0.95+5*math.sqrt(signal)
    rebin = int(0.65*math.sqrt(signal))
    ranges = [ (range_min,range_max,8) ] # 0.5 works fine 
    print(ranges)

    if doSymmetric:
      print("DOING SYMMETRICAL FITS")
      #  initvals = [[0.5, 1.5, 10, 10, signal*0.95, signal/8,0.1] ]
      #  #150 # [0.49417204075560367, 1.5100260010637723, 150868999.80378264, 6.263211893969833, 142.5, 18.75, 0.10038296935781954]
      ...
      initvals = [[1, 1, 10, 10, signal*0.9, signal/3, signal/3 , 0.06] ]
      limits = [
        [0.5, 3],
        [0.5, 3],
        [0.1, 200],
        [0.1, 200],
        [signal*0.85, signal*1.1],
        [10, signal - range_min],
        [10, range_max - signal],
        [0, 10]
      ]
    ...
    f = ROOT.TFile(args.folder+"/systematic_updown_mjj_MGPy8EG_S1_qqa_Ph25_mRp"+str(signal)+"_gASp1_qContentUDSC.root")
   ...
   # Strong reduction JES, Full JER:
    ...
    for i,hist in enumerate(hists):
        ...
        for j,variation in enumerate(variations):
            ...
            # only mean, width and normalization floating freely (for the syst, for nominal get a good set of parameters 4 5 6 and use them fixed):
            if j == 0:
               for k in range(len(limits)):
            ...
            h.GetXaxis().SetRangeUser(signal*0.1,signal*2)
            h.Draw("e")
            h.SetMinimum(0)
            ...
            f1.SetLineStyle(2)  # 2 = dashed/dotted
            f1.SetLineWidth(2)
            f1.Draw("same")
            

            f1_fill = f1.Clone("f1_fill")
            f1_fill.SetLineColor(ROOT.kRed)
            f1_fill.SetLineStyle(1)
            f1_fill.SetLineWidth(4)
            f1_fill.SetRange(ranges[i][0],ranges[i][1])
            f1_fill.Draw("same")
            ...
            # temporary fix to 150 mass DSCB parametrization
            if signal == 150:
                f1_fix = ROOT.TF1("dscb fixed", doubleSidedCrystalBall, 0, 3000, 7)
                f1_fix.SetNpx(1000)
                npar = f1.GetNpar()
                for ii in range(npar):
                   f1_fix.SetParameter(ii, f1.GetParameter(ii));
                f1_fix.SetParameter(3,0.3)
                f1_fix.SetLineColor(ROOT.kBlue)
                f1_fix.SetLineStyle(2)
                f1_fix.SetLineWidth(2)
                f1_fix.SetRange(x_alpha_high,1000)
                f1_fix.Draw("same")
            ...
            if doLegend:
               ...
               leg.AddEntry(f1_fill, "DSCB fit","l")
