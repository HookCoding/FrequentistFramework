...
void ATLASLabel(Double_t x,Double_t y,const char* text,Int_t align,Color_t color) 
{
  ...
  if (gPad->GetWw() < gPad->GetWh())
    delx = 0.1*(gStyle->GetTextSize()/25.)*gPad->GetWh()/472;  //0.115*696*gPad->GetWh()/(472*gPad->GetWw());
  else
    delx = 0.1*(gStyle->GetTextSize()/25.)*gPad->GetWw()/696;  //0.115*696*gPad->GetWh()/(472*gPad->GetWw());
