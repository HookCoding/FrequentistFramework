...
def _prepare_build_fit_extract_success_doubles(
    ...
    # build_fit_extract() only reaches ROOT.TFile / PostfitExtractor /
    ...
    monkeypatch.setattr(module.ROOT, "TFile", lambda datafile: _FakeTFile(first_bin), raising=False)

    _FakePostfitExtractor.instances = []
    _FakeFitParameterExtractor.instances = []
    monkeypatch.setattr(module, "PostfitExtractor", _FakePostfitExtractor)
    monkeypatch.setattr(module, "FitParameterExtractor", _FakeFitParameterExtractor)
...
def test_build_fit_extract_succeeds_for_unmasked_fit(
    ...
    module = _load_run_anafit_module(monkeypatch)
    executed_commands = _prepare_build_fit_extract_success_doubles(module, monkeypatch)

    result = module.build_fit_extract(
        topfile="top.xml",
        datafile="input.root",
        datahist="data",
        rangelow=481,
...
def test_build_fit_extract_succeeds_for_masked_fit(
    ...
    module = _load_run_anafit_module(monkeypatch)
    _prepare_build_fit_extract_success_doubles(module, monkeypatch)

    quickfit_commands: list[str] = []
    original_execute_required = module.execute_required
    ...
    monkeypatch.setattr(module, "execute_required", execute_required_recording_quickfit)
